    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 font-headline">مرحباً، {user?.username}</h1>
                    <p className="text-slate-500 mt-1">إليك ملخص أداء المتجر لهذا اليوم</p>
                </div>
                <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100 flex items-center gap-2">
                    <span className="w-2 h-2 bg-secondary rounded-full animate-pulse"></span>
                    <span className="text-xs font-bold text-slate-600 uppercase tracking-widest">النظام متصل محلياً</span>
                </div>
            </div>

            {/* KPI Cards */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard title="مبيعات اليوم" value={`${stats.todaySales.toFixed(2)} ر.س`} icon="payments" color="bg-primary-container/10 text-primary" />
                <StatCard title="إجمالي المنتجات" value={stats.productCount} icon="inventory_2" color="bg-secondary-container/20 text-secondary-container" />
                        ...
                        {stats.recentSales.length > 0 ? stats.recentSales.map((sale, i) => (
                            <div key={i} className="px-6 py-4 flex justify-between items-center hover:bg-slate-50 transition-colors">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-slate-800">عملية بيع جديدة</p>
                                        <p className="text-[10px] text-slate-400 uppercase font-bold">{sale.time} • {sale.method}</p>
                                    </div>
                                </div>
                                <span className="font-black text-primary">{sale.total.toFixed(2)} ر.س</span>
                            </div>
                        )) : (
                            <div className="p-12 text-center text-slate-400 italic">لا توجد مبيعات مسجلة اليوم</div>
                        )}
                    </div>
                </div>
            </div>
...
const StatCard = ({ title, value, icon, color, isCritical }) => (
    <div className={`bg-white p-6 rounded-2xl shadow-sm border ${isCritical ? 'border-error/20' : 'border-slate-100'} flex items-center gap-5 transition-transform hover:scale-[1.02]`}>
